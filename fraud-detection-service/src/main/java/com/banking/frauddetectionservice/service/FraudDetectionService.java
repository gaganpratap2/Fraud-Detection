...
public class FraudDetectionService {
    ...
    public void checkTransaction(Map<String, Object> payload) {
        ...
        FraudCheckResult res =
                performFraudChecks(accountNumber, amount, senderBalance);

        if (res.isFraud()) {

            log.info(
                    "Suspicious activity detected - amount: {} reason: {} - requesting OTP verification",
                    amount,
                    res.getReason()
            );
    ...
    private boolean isVelocityExceeded(String accountNumber) {
        ...
        log.info(
                ...
                accountNumber,
                count,
                maxTransactionPerMinute
        );

        return count != null
                && count > maxTransactionPerMinute;
    }
